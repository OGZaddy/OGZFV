...
// KRAKEN ADAPTER - LIVE TRADING INTEGRATION
const KrakenAdapterSimple = require('./kraken_adapter_simple');

// TIER FEATURE FLAGS - SUBSCRIPTION MANAGEMENT
...
// TierFeatureFlags already imported above
...
class OGZPrimeV14Final {
  constructor() {
    ...
    const tier = process.env.SUBSCRIPTION_TIER || 'elite'; // Can be 'starter', 'pro', or 'elite'
    ...
    // OHLCV Candle storage for proper technical analysis (Codex fix)
    this.candles = [];
    this.currentCandle = null;
    this.candlePeriod = 60000; // 1 minute candles
   ...
   */
  connectWebSocket() {
    const wsUrl = 'ws://127.0.0.1:3010/ws'; // Connect to unified WebSocket server
  ...
  async initialize() {
    ...
    try {
      ...
      // WebSocket connection enabled for dashboard communication
  ...
  async initializeEnhancedSystems() {
    ...
    // Optimized Indicators - Fast technical indicator calculations (singleton)
    this.optimizedIndicators = OptimizedIndicators;
  ...
  async initializeNetworkServices() {
    ...
    // Start HTTP server - DISABLED (using unified server on port 3010)
  ...
  async startTradingOperations() {
    ...
    // CONNECT TO KRAKEN BEFORE TRADING
    console.log('🔗 Connecting to Kraken...');
    const connected = await this.krakenAdapter.connect();
    if (!connected) {
  ...
  async performTradingCycle() {
    ...
    try {
      ...
      const maxSimultaneousPositions = this.tierFlags.getValue('maxPositions') || 1;
  ...
  async getMarketData() {
    ...
    if (this.cachedMarketData && this.lastDataReceived && this.cachedMarketData.price) {
      ...
      if (dataAge < maxAge) {
        ...
        // Codex fix: Convert tick to OHLCV candle
        this.updateOHLCVCandle(this.cachedMarketData.price);
  ...
  calculateRSI(priceData, period = 14) {
    ...
    if (priceData.length < minCandles) return 50;
  ...
  async analyzePatterns(marketData) {
    ...
    try {
      // RSI patterns - NORMALIZED
      if (marketData.rsi < 35) {
        const pattern = {
          type: 'rsi_buy',  // Normalized from 'oversold'
          direction: 'buy',
        ...
        const pattern = {
          type: 'rsi_sell',  // Normalized from 'overbought'
          direction: 'sell',
      ...
      if (marketData.macd > 50) {
        const pattern = {
          type: 'macd_buy',  // Normalized from 'bullish'
          direction: 'buy',
        ...
        const pattern = {
          type: 'macd_sell',  // Normalized from 'bearish'
          direction: 'sell',
      ...
      if (marketData.volatility > 0.03) {
        const pattern = {
          type: marketData.trend === 'up' ? 'volatility_buy' : 'volatility_sell',  // Normalized
   ...
   * DIRECTIONAL VERSION - Separate bullish/bearish confidence scores
   */
  calculateRealConfidence(marketData, patterns = []) {
    ...
    let confidence = 0; // Fix: Declare the confidence variable
    ...
    if (patterns && patterns.length > 0) {
      patterns.forEach(pattern => {
        ...
        if (pattern.type && (pattern.type.includes('buy') || pattern.type.includes('bullish') ||
          ...
          bullishConfidence += patternBonus * qualityMultiplier;
          ...
          bearishConfidence += patternBonus * qualityMultiplier;
        }
      });
    }
    ...
    finalConfidence = Math.max(0, Math.min(0.95, finalConfidence));
  ...
  /**
   * 🎯 Determine trading direction (NOW USES DIRECTIONAL CONFIDENCE)
   */
  determineTradingDirection(marketData, patterns, confidence) {
    try {
      // CRITICAL FIX: Use directional scores from marketData
      if (marketData.direction && marketData.direction !== 'neutral') {
        console.log(`📊 Using directional signal: ${marketData.direction} (Bull: ${(marketData.bullishScore * 100).toFixed(1)}%, Bear: ${(marketData.bearishScore * 100).toFixed(1)}%)`);
        return marketData.direction;
      }

      // Fallback to pattern-based direction if no clear directional signal
  ...
  /**
   * 🕯️ Convert tick stream to OHLCV candles (Codex fix)
   */
  updateOHLCVCandle(price, timestamp = Date.now()) {
    const candleStart = Math.floor(timestamp / this.candlePeriod) * this.candlePeriod;

    if (!this.currentCandle || this.currentCandle.timestamp !== candleStart) {
      // Start new candle
      if (this.currentCandle) {
        // Close previous candle
        this.currentCandle.c = price; // Final close
  ...
  calculatePriceChange(marketData) {
    try {
      // Codex fix: Use .c (close) field if available, otherwise use price
      const currentPrice = marketData.c || marketData.close || marketData.price;

      if (this.priceHistory.length < 2) {
        return 0; // No history to compare
      }

      // CRITICAL FIX: priceHistory stores objects {c, t}, not raw numbers!
      // Must access the .c field to get the actual price value
      const previousPriceObj = this.priceHistory[this.priceHistory.length - 2];
      const previousPrice = previousPriceObj.c || previousPriceObj.close || previousPriceObj.price;
  ...
  async executeTrade(direction, positionSize, confidence, marketData, patterns = []) {
    try {
      ...
      // CRITICAL FIX: Reserve capital for position
      const reservedAmount = position.tradeValue || 0;
      this.systemState.currentBalance -= reservedAmount;
      ...
      console.log(`💰 RESERVED: $${reservedAmount.toFixed(2)} from balance`);
      ...
      // Update system state (increment trade count on ENTRY only)
      ...
      if (!this.config.simulate) {
        ...
        // KRAKEN INTEGRATION - LIVE TRADING
        try {
          if (this.krakenAdapter && this.krakenAdapter.connected) {
            const order = {
              symbol: 'BTC-USD',
            ...
            console.log(`📡 Placing ${direction} order: ${positionSize} BTC`);
            const result = await this.krakenAdapter.placeOrder(order);
            console.log(`✅ Order placed successfully: ${result.orderId}`);

            // Store order ID for tracking
            this.lastOrderId = result.orderId;
          } else {
            console.log('❌ Kraken adapter not connected - cannot place real trades');
          }
        } catch (error) {
  ...
  async closePosition(tradeId, exitPrice, reason) {
    try {
      ...
      const reservedAmount = position.tradeValue || 0;
      this.systemState.currentBalance += reservedAmount; // Return reserved capital
      this.systemState.currentBalance += pnl; // Add net P&L
