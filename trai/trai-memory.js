...
class MoverMemory extends EventEmitter {
  ...
  async ingestCategory(categoryPath, categoryName) {
    try {
      ...
      for (const file of files) {
        if (file.endsWith('.md')) {
          ...
          const insights = this.extractInsights(content, file);
          
          // Store in long-term memory
          this.updateLongTermMemory(categoryName, file.replace('.md', ''), {
      ...
      console.log(`[MoverMemory] Ingested ${documentCount} documents from ${categoryName}`);
      return documentCount;
    } catch (error) {
      console.error(`[MoverMemory] Failed to ingest category ${categoryName}:`, error);
      return 0;
    }
  }

  extractInsights(content, filename) {
    const insights = [];
    
    // Extract trading-related keywords
    const tradingKeywords = ['trading', 'bot', 'profit', 'loss', 'strategy', 'market', 'buy', 'sell'];
    tradingKeywords.forEach(keyword => {
      if (content.toLowerCase().includes(keyword)) {
        insights.push({ type: 'trading', keyword });
      }
    });
    
    // Extract emotional context
    const emotionalKeywords = ['frustrated', 'excited', 'confident', 'worried', 'optimistic'];
    emotionalKeywords.forEach(keyword => {
      if (content.toLowerCase().includes(keyword)) {
        insights.push({ type: 'emotional', keyword });
      }
    });
    
    // Extract technical terms
    const techKeywords = ['api', 'websocket', 'polygon', 'ssl', 'server', 'code'];
    techKeywords.forEach(keyword => {
      if (content.toLowerCase().includes(keyword)) {
        insights.push({ type: 'technical', keyword });
      }
    });
