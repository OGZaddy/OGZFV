...
class AdvancedWebSocketBroadcastSystem extends EventEmitter {
  constructor(options = {}) {
    ...
    this.connections = new Map(); // Added for SSL server compatibility
  ...
  registerClient(ws, metadata = {}) {
    ...
    this.connections.set(clientId, clientInfo); // For SSL server compatibility
   ...
   */
  sendDirect(client, message, options = {}) {
    return this.sendToClient(client, message, options);
  }
  ...
  removeClient(clientId) {
    ...
    this.connections.delete(clientId); // For SSL server compatibility
