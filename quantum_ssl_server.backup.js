...
// 🤖 IMPORT TRAI ENHANCED AI CLONE
const TraiEnhanced = require('./trai/trai-enhanced');
...
// Initialize TRAI ENHANCED with persistent memory and CodeLlama bridge
const trai = new TraiEnhanced({
  memoryPath: '/root/OGZFV-valhalla/data/trai-memory',
  learningRate: 0.8,
  responseStyle: 'elite_trader'
});

// Initialize Trai Enhanced system
trai.initialize().then(success => {
  if (success) {
    console.log('🧠 Trai Enhanced AI Clone fully loaded and ready for data pipeline');
    console.log('🚀 CodeLlama 70B bridge status:', trai.getStatus().desktop.connected ? 'CONNECTED' : 'TUNNEL NEEDED');
    
    // Add data feeding capabilities
    setupTraiDataFeeding();
  } else {
    console.error('❌ Failed to initialize Trai Enhanced');
  }
}).catch(console.error);

// Simple data feeding for Trai
function setupTraiDataFeeding() {
  console.log('📡 Setting up Trai data pipeline...');
  
  // Listen for trading data broadcasts
  broadcaster.on('broadcast_sent', async (data) => {
    if (data.type === 'trade_update' || data.type === 'metrics_update' || data.type === 'equity_update') {
      await feedDataToTrai(data.type, data.data);
    }
  });
  
  // Periodically scan reports directory
  setInterval(() => {
    scanReportsForTrai();
  }, 30000); // Every 30 seconds
}

async function feedDataToTrai(dataType, data) {
  let contextualInput = '';
  
  switch(dataType) {
    case 'trade_update':
      contextualInput = `Trade executed: ${data.symbol || 'BTC'} ${data.action || 'unknown'} - P&L: ${data.pnl || 'calculating'}`;
      break;
    case 'metrics_update':  
      contextualInput = `Performance update - Win rate: ${data.winRate || 0}%, Total trades: ${data.totalTrades || 0}`;
      break;
    case 'equity_update':
      contextualInput = `Portfolio: $${data.totalEquity || 0}, Daily change: ${data.dailyChange || 0}%`;
      break;
  }
  
  if (contextualInput) {
    try {
      const response = await trai.query(contextualInput, {
        type: 'trading_data',
        timestamp: Date.now()
      });
      console.log(`🧠 Trai processed: ${dataType} -> Response generated`);
    } catch (error) {
      console.error('Error feeding data to Trai:', error);
    }
  }
}

async function scanReportsForTrai() {
  try {
    const reportsPath = path.join(__dirname, 'reports');
    
    for (const tier of ['pro', 'elite', 'starter', 'quantum']) {
      const tierPath = path.join(reportsPath, tier);
      
      try {
        const files = await fs.promises.readdir(tierPath);
        const recentFile = files
          .filter(f => f.includes('metrics') || f.includes('equity'))
          .sort()
          .pop(); // Get most recent
          
        if (recentFile) {
          const filePath = path.join(tierPath, recentFile);
          const stats = await fs.promises.stat(filePath);
          const isRecent = (Date.now() - stats.mtime.getTime()) < 300000; // 5 minutes
          
          if (isRecent) {
            const data = JSON.parse(await fs.promises.readFile(filePath, 'utf8'));
            feedDataToTrai(recentFile.includes('metrics') ? 'metrics_update' : 'equity_update', data);
          }
        }
      } catch (err) {
        // Tier directory might not exist, continue
      }
    }
  } catch (error) {
    // Reports directory scan failed, but don't crash
  }
}
...
(async () => {
  try {
    console.log('🤖 Initializing Trai AI Clone...');
    await trai.initialize();
...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', async (message) => {
    try {
      ...
      // Handle Trai AI chat messages
      if (data.type === 'trai_chat') {
        console.log('💬 Trai chat message received:', data.message);
        
        try {
          const response = trai.generateResponse(data.message, {
            contextType: 'dashboard_chat',
            timestamp: Date.now(),
          ...
          console.log('🧠 Trai response sent to dashboard');
        } catch (error) {
          console.error('Error generating Trai response:', error);
          ws.send(JSON.stringify({
            type: 'trai_response', 
            message: "Sorry, I'm processing a lot of market data right now. Try again in a moment.",
            timestamp: Date.now()
          }));
        }
        return;
      ...
      // Handle trade messages from bots (pro, starter, elite, quantum)
      if (data.type === 'trade' && data.source === 'trading_bot') {
        console.log(`💰 ${data.botTier?.toUpperCase()} BOT TRADE - Broadcasting to dashboard`);
        broadcaster.broadcast(data);
        
        // Log trade details
        console.log(`   ${data.action?.toUpperCase()} @ $${data.price} | Confidence: ${data.confidence}% | P&L: $${data.pnl || 0}`);
      }
      ...
      // 🤖 Handle Trai chat commands
      if (data.op === 'cmd' && data.action === 'trai:chat') {
        try {
          const text = String(data.args?.text || '').trim();
          console.log(`🤖 Trai received: "${text}"`);
          
          if (!text) {
            ws.send(JSON.stringify({ op: 'ack', action: 'trai:chat', ok: false, error: 'Empty message' }));
            return;
          }
          ...
          // Send acknowledgment
          ws.send(JSON.stringify({ op: 'ack', action: 'trai:chat', ok: true }));
      ...
      if (data.op === 'cmd' && data.action === 'trai:status') {
        try {
          const status = {
            initialized: true,
            personality: trai.config.personality,
