// 21_validate_and_startup.js - COMPLETE SYSTEM VALIDATION AND STARTUP
...
const VALIDATION_CHECKS = {
  ...
  commissionFixed: {
    ...
    check: (content) => content.includes('positionSizeUSD * fee') || 
                        content.includes('positionSize * fee'),
  ...
  websocketSingleton: {
    ...
    check: (content) => content.includes('singletonInstance') || 
                        content.includes('WebSocketManager.instance'),
...
async function validateSystem() {
  ...
  for (const [checkName, checkConfig] of Object.entries(VALIDATION_CHECKS)) {
    const filePath = path.join(PROJECT_ROOT, checkConfig.file);
...
async function startSystem(options = {}) {
  ...
  const launcherPath = path.join(PROJECT_ROOT, 'run-trading-bot-v13-quantum.js');
...
async function checkSystemStatus() {
  ...
  const checks = {
    'WebSocket Server (3010)': await checkPort(3010),
    'API Server (3011)': await checkPort(3011),
    'Polygon Connection': checkPolygonConnection(),
    'Module Integration': checkModuleIntegration(),
    'Dashboard Available': checkDashboard()
  };
...
function checkModuleIntegration() {
  ...
  const integrationPath = path.join(__dirname, '16_master_integration.js');
  return fs.existsSync(integrationPath);
}

function checkDashboard() {
  const dashboardPath = path.join(PROJECT_ROOT, 'unified-dashboard.html');
...
async function main() {
  ...
  switch (command) {
    ...
    default:
      console.log('Usage: node 21_validate_and_startup.js [command]');
