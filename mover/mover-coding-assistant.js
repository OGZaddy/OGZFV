// ==========================================
// THE MOVER - AI CODING ASSISTANT
// Local LLM + OGZ System Knowledge + Personality
// ==========================================

const axios = require('axios');
const TheMoverAI = require('./the-mover-ai-clone.js');

class MoverCodingAssistant {
  constructor() {
    this.mover = new TheMoverAI();
    ...
    console.log('[MoverCoder] AI Coding Assistant initializing...');
  }

  async initialize() {
    try {
      // Initialize The Mover personality
      await this.mover.initializeFinalForm();
      console.log('[MoverCoder] Personality loaded - 277 patterns ready');
      
      // Test Ollama connection
      const modelStatus = await this.testOllamaConnection();
      if (modelStatus.success) {
        console.log(`[MoverCoder] Connected to ${this.currentModel}`);
        this.systemInitialized = true;
        return true;
      } else {
        console.error('[MoverCoder] Ollama connection failed:', modelStatus.error);
        return false;
      }
      
    } catch (error) {
      console.error('[MoverCoder] Initialization failed:', error);
      return false;
    }
  }
  ...
  async processCodeRequest(request) {
    ...
    try {
      // Step 1: Get Mover personality response
      const personalityResponse = this.mover.generateResponse(request);
      ...
      console.error('[MoverCoder] Processing error:', error);
      return { success: false, error: error.message };
    }
  }
...
Generate the technical response:`;
  ...
  async switchModel(modelName) {
    ...
    if (testResult.success) {
      console.log(`[MoverCoder] Switched to ${modelName}`);
  ...
  getSystemStatus() {
    return {
      ...
      personalityPatterns: this.mover.learningStats?.totalPatterns || 0,
...
module.exports = MoverCodingAssistant;